...
// Api Caller for View Controller

class FlickrPhotoSearch: PhotosViewControllerProtocol{
    ...
    func searchPhotos(forSearchString searchString: String, pageNumber: Int, andItemsPerPage itemsPerPage: Int, completion: @escaping ([Photo]?, String?)-> Void){
        ...
        PhotoNetworkManager.shared.request(withURL: flickrSearchURL, completion:{[weak self] (data, error) in
            if let _ = error {
